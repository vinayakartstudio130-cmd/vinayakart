      ...
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8">
              ...
              <div className="flex items-center gap-2 mb-6">
                {socials.map((s) => (
                  <a
                    ...
                    href={s.href}
                    aria-label={s.label}
                    className="w-9 h-9 flex items-center justify-center text-white/40 hover:text-[#1CB8D2] transition-all duration-300 rounded-lg group"
              ...
              <div className="flex flex-col gap-2">
                {['Govt. of India Registered', 'ASI Compliant', '25 Years Heritage'].map((badge) => (
                  <span
                    ...
                    style={{ border: '1px solid rgba(28,184,210,0.15)', color: 'rgba(255,255,255,0.25)', background: 'rgba(28,184,210,0.04)' }}
                  >
                    {badge}
                  </span>
                ))}
            ...
            {Object.entries(footerLinks).map(([heading, links]) => (
              <div key={heading} className="md:col-span-2">
                <h4 className="font-sans text-[10px] font-bold tracking-[0.22em] uppercase mb-5" style={{ color: '#1CB8D2' }}>
                  {heading}
                </h4>
                <ul className="space-y-2.5">
                  {links.map((link) => (
                    <li key={link}>
                      <a href="#" className="font-sans text-[12px] leading-relaxed transition-colors duration-300 hover:text-[#1CB8D2]" style={{ color: 'rgba(255,255,255,0.3)' }}>
                        {link}
                      </a>
                    </li>
                  ))}
        ...
        <div className="section-container py-5">
          <div className="flex flex-col md:flex-row items-center justify-between gap-3">
            <p className="font-sans text-[11px] tracking-[0.08em]" style={{ color: 'rgba(255,255,255,0.2)' }}>
            ...
            <div className="flex items-center gap-5">
              {['Privacy Policy', 'Terms of Service', 'Shipping Policy'].map((link) => (
                <a key={link} href="#" className="font-sans text-[11px] transition-colors hover:text-[#1CB8D2]" style={{ color: 'rgba(255,255,255,0.2)' }}>
                  {link}
                </a>
              ))}
            </div>
